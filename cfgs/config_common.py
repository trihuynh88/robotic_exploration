...
def get_navtask_vars(navtask_str):
  ...
  ks_all = ['dataset_name', 'modality', 'task', 'history', 'max_dist',
            'num_steps', 'step_size', 'n_ori', 'aux_views', 'data_aug', 'explore_radius']
  ks = ks_all[:len(vals)]
  ...
  # num steps
  if len(vals) == 5: ks.append('num_steps'); vals.append('1001')  #was 40 originally
  # step size
  ...
  if len(vals) == 10: ks.append('explore_radius'); vals.append('4')
...
def process_navtask_str(navtask_str):
  ...
  navtask.task_params.explore_radius=int(navtask_vars.explore_radius)
