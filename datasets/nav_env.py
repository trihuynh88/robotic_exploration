...
class NavigationEnv(GridWorld, Building):
  ...
  def _preprocess_for_task(self, seed):
    ...
    if self.task is None or self.task.seed != seed:
      ...
      #Tri
      #gtG, nodes, nodes_to_id = convert_to_graph_tool(G)
      gtG, nodes, nodes_to_id, id_to_nodes = convert_to_graph_tool(G)
      self.task.gtG = gtG
      self.task.nodes = nodes
      self.task.delta_theta = 2.0*np.pi/(self.task.n_ori*1.)
      self.task.nodes_to_id = nodes_to_id
      #Tri
      self.task.id_to_nodes = id_to_nodes
  ...
  def take_action_and_explore(self,current_node_ids,action):
    ...
    #pdb.set_trace()
    new_node_ids = GridWorld.take_action(self, current_node_ids, action)
    new_nodes = [list(self.task.id_to_nodes[x]) for x in new_node_ids]
    new_nodes = np.asarray(new_nodes)
    xyt = self.to_actual_xyt_vec(new_nodes)
    m = self.unexplored.shape[1]
    n = self.unexplored.shape[2]
    rewards = np.zeros(xyt.shape[0])
